...
CREATE OR REPLACE PACKAGE BODY STAR_SCHEME_PACKAGE AS
    ...
    -- PROCEDURA WSTAWIAJĄCA DANE DO TABELI FAKTÓW
    PROCEDURE P_INIT_SALES_FACT IS
    BEGIN
        MERGE INTO SALES_FACT SF
        USING (SELECT DISTINCT TD.TIME_ID,
                               PD.PRODUCT_ID,
                               LD.LOCALIZATION_ID,
                               SUM(OP.SALES)  SUM_SALES,
                               SUM(OP.PROFIT) SUM_PROFITS
               FROM ORDERPRODUCTS OP
                        ...
                        JOIN TIME_DIM TD ON TD.SELL_DATE = O.ORDER_DATE
                        JOIN PRODUCTS_DIM PD ON P.PRODUCT_NAME = PD.NAME AND C.CATEGORY_NAME = PD.CATEGORY_NAME
                        JOIN LOCALIZATIONS_DIM LD
                             ON (LD.COUNTRY = L.COUNTRY AND LD.STATE = L.STATE AND LD.CITY = L.CITY)
               GROUP BY TD.TIME_ID, PD.PRODUCT_ID, LD.LOCALIZATION_ID) SUB

        ON (
            SF.TIME_ID = SUB.TIME_ID
                AND SF.PRODUCT_ID = SUB.PRODUCT_ID
                AND SF.LOCALIZATION_ID = SUB.LOCALIZATION_ID
                AND SF.VALUE_M = SUB.SUM_SALES
                AND SF.PROFIT_M = SUM_PROFITS
            )
        WHEN NOT MATCHED THEN
            INSERT (SF.TIME_ID, SF.PRODUCT_ID, SF.LOCALIZATION_ID, SF.VALUE_M, SF.PROFIT_M)
            VALUES (SUB.TIME_ID, SUB.PRODUCT_ID, SUB.LOCALIZATION_ID, SUB.SUM_SALES, SUB.SUM_PROFITS);
    END P_INIT_SALES_FACT;
END STAR_SCHEME_PACKAGE;
/

-- WYWOŁANIE WSZYSTKICH PROCEDUR INICJUJĄCYCH Z PACZKI
BEGIN
    STAR_SCHEME_PACKAGE.P_INIT_TIME_DIM;
    STAR_SCHEME_PACKAGE.P_INIT_LOCALIZATIONS_DIM;
    STAR_SCHEME_PACKAGE.P_INIT_PRODUCTS_DIM;
    STAR_SCHEME_PACKAGE.P_INIT_SALES_FACT;
END;
/
