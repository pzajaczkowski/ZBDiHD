...
BEGIN
    AMAZON_PACKAGE.P_LOAD_LOCALIZATIONS;
    AMAZON_PACKAGE.P_LOAD_CUSTOMERS;
    AMAZON_PACKAGE.P_LOAD_CATEGORIES;
    AMAZON_PACKAGE.P_LOAD_PRODUCTS;
    AMAZON_PACKAGE.P_LOAD_ORDERS;
    AMAZON_PACKAGE.P_LOAD_ORDERPRODUCTS;
END;
...
SELECT STATE,
       ...
       YEAR
FROM (SELECT TRIM(REGEXP_SUBSTR(T.GEOGRAPHY, '[^,]+', 1, 3)) AS STATE, T.PROFIT, EXTRACT(YEAR FROM T.ORDER_DATE) AS YEAR
      FROM TEMP T)
GROUP BY STATE, YEAR
ORDER BY YEAR, PROFIT DESC;

SELECT COUNT(1) AS LOCALIZATIONS
FROM LOCALIZATIONS;
SELECT COUNT(1) AS CUSTOMERS
FROM CUSTOMERS;
SELECT COUNT(1) AS CATEGORIES
FROM CATEGORIES;
SELECT COUNT(1) AS PRODUCTS
FROM PRODUCTS;
SELECT COUNT(1) AS ORDERS
FROM ORDERS;
SELECT COUNT(1) AS ORDERPRODUCTS
FROM ORDERPRODUCTS;

-- PRODUCTS AND CATEGORY SHOULD BE CONNECTED VIA MANY TO MANY TABLE?